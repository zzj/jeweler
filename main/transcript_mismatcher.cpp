...
TranscriptMismatcherAnalyzer::TranscriptMismatcherAnalyzer(string filename,
    ...
    for (size_t i=0; i < jeweler_info->gene_id.size(); i++) {
        ...
        if (numtestcase > 0 && i == numtestcase) {
            break;
        }
    }
    end_loading();
...
void TranscriptMismatcherAnalyzer::analyze() {
    ...
    calculate_error<char>(read_qualities, read_mismatch_qualities,
                          num_calls_by_quality,
                          num_calls_by_quality_baseline,
                          num_mismatches_by_quality,
                          num_mismatches_by_quality_baseline,
                          error_rate_by_quality
                         );
    unique_ptr<Jeweler::TranscriptMismatcherAnalyzerData>
        tmad(new Jeweler::TranscriptMismatcherAnalyzerData);
    dump_error_rate_by_quality (tmad->add_iteration_result());
    do {
        calculate_error<char>(read_qualities, read_mismatch_qualities,
                              num_calls_by_quality,
                              num_calls_by_quality_baseline,
    ...
    dump_location_results(tmad.get());
    dump_protobuf_data(this->filename, tmad.get());
}

void TranscriptMismatcherAnalyzer::dump_error_rate_by_quality(
        Jeweler::TranscriptMismatcherAnalyzerData::IterationResult* data) {
    string phred;
    for (auto i = num_calls_by_quality.begin();
          i !=  num_calls_by_quality.end();
          i ++) {
        phred += i->first;
        data->add_num_calls(i->second);
        data->add_num_mismatches(num_mismatches_by_quality[i->first]);
        data->add_error_rate(error_rate_by_quality[i->first]);
    }
    data->set_phred(phred);
}

void TranscriptMismatcherAnalyzer::dump_location_results(Jeweler::TranscriptMismatcherAnalyzerData *data) {
    int i;
    for (i = 0; i < num_locations; i ++) {
        if (!is_consistent_mismatches.test(i)) continue;
        Jeweler::TranscriptMismatcherAnalyzerData::LocationResult *lr =
            data->add_location_result();
        lr->set_gene_id(gene_ids[i]);
        lr->set_genome_location(genome_locations[i]);
        lr->set_pvalue(p_values[i]);
        lr->set_coverage(coverages[i]);
        lr->set_num_mismatches(mismatches[i]);
    }
}
