plot.comparision <- function(data, name, filename, title, so, founder.names, label.position, break.position, offset.position){
  ...
  plot(x,y1,xlab = "samples", ylab ="number", main = paste(title, "mean(change) =",sprintf("%.2f%%",mean(ratio)*100)) , pch = 15, col ="blue", ylim=c(min(c(y1,y2)) -l *0.1, max(c(y1,y2)) + l *0.15),  xaxt="n")
  ...
  axis(1, at=label.position,labels=founder.names, las=1)
...
order.founder = c("FG","FG", "GF", "GF", "FH", "FH", "HF", "HF", "GH","GH" ,"HG", "HG")
order.founder.alias = c("CP","CP", "PC", "PC", "CW", "CW", "WC", "WC", "PW","PW" ,"WP", "WP")
order.gender = c("F","M","F","M","F","M","F","M","F","M","F","M")
order.names = c()
...
plot.comparision(cuff,"exact", "exact", '(b) Number of  reported transcripts that exactly matched \n an annotated one\n',
                 sample.order,
                 order.names,
                 label.position,
                 break.position,
...
plot.comparision(cuff,"pseudo", "pseudo", '(c) Number of annotated pseudogenes reported\n',
                 sample.order,
                 order.names,
                 label.position,
                 break.position,
...
plot.comparision(cuff,"unknown", "unknown", '(d) Number of unannotated transcripts reported \n',
                 sample.order,
                 order.names,
                 label.position,
                 break.position,
...
plot.comparision(cuff,"total", "total",'(a) Number of total transcripts reported\n',
                 sample.order,
                 order.names,
                 label.position,
                 break.position,
